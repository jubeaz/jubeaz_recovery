...
SLIVER_COMPILE_PATH=/root/sliver
SLIVER_SERVER_NAME=sliver-server
SLIVER_CLIENT_NAME=sliver-client
SLIVER_CLIENT_ALIAS=sliver
SLIVER_SERVER_INSTALL_PATH=/root
SLIVER_CLIENT_INSTALL_PATH=/usr/local/bin

if test -f "$SLIVER_COMPILE_PATH/$SLIVER_SERVER_NAME"; then
    #mkdir -p $SLIVER_SERVER_INSTALL_PATH
    echo "Moving the Sliver server executable to /root/sliver-server..."
    mv "$SLIVER_COMPILE_PATH/$SLIVER_SERVER_NAME" $SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME

    echo "Setting permissions for the Sliver server executable..."
    chmod 755 $SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME

    echo "Unpacking the Sliver server..."
    $SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME unpack --force
fi

if test -f "$SLIVER_COMPILE_PATH/$SLIVER_CLIENT_NAME"; then
    echo "Setting permissions for the Sliver client executable..."
    chmod 755 "$SLIVER_COMPILE_PATH/$SLIVER_CLIENT_NAME" $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_NAME

    echo "Copying the Sliver client executable to $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_NAME..."
    cp -vv "$SLIVER_COMPILE_PATH/$SLIVER_CLIENT_NAME" $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_NAME

    echo "Creating a symbolic link for sliver-client at $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_ALIAS..."
    ln -sf $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_NAME $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_ALIAS

    echo "Setting permissions for the symbolic link $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_ALIAS..."
    chmod 755 $SLIVER_CLIENT_INSTALL_PATH/$SLIVER_CLIENT_ALIAS
fi
...
RestartSec=3
User=root
ExecStart=$SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME daemon
...
systemctl enable sliver # enable the service now
...
$SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME operator --permissions all --name root --lhost localhost --save /root/.sliver-client/configs
...
for USER_DIR in "${USER_DIRS[@]}"; do
    ...
    if id -u "$USER" >/dev/null 2>&1; then
        ...
        $SLIVER_SERVER_INSTALL_PATH/$SLIVER_SERVER_NAME operator --name "$USER" --permissions all --lhost localhost --save "$USER_DIR/.sliver-client/configs"
